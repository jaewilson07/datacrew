...
const paint_app = async (
  ...
  // paintTable
  var tableEl = document.getElementById("tableList");
  ...
  show_export_button = await isViewerTheOwner(collectionService);
...
function CollectionService(collection, columnsObject, syncToDataset) {
  ...
  function getCollectionInfo() {
    if (collectionInfoPromise == null) {
      collectionInfoPromise = window.domo.get(
  ...
  function runSync() {
    ...
    return window.domo.post(`/domo/datastores/v1/export`).then((res) => {
