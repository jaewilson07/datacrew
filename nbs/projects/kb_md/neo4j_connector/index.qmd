...
* Whitelist Domo's IP addresses that can be found at [Whitelisting IP addresses for connectors](/s/article/360043630093)
...
| Field | Description |
| --- | --- |
| Neo4j Instance URL | Enter the URL of your instance.
Example: neo4j.database.url or [graph.site.com](http://graph.site.com):7473.


 

**Note**: Include port number if applicable. The default port is 7473 for Neo4j.


 |
| Neo4j Database Name | Enter the name of your database. |
...
| Menu | Description |
| --- | --- |
| Query | Enter your Cypher query.
**Example**:* MATCH (p:Person)-[:BUYS]->(i:Item)

       RETURN [p.name](http://p.name) as Name,i.type as ItemType* MATCH (a:Actor)-[:PRODUCES]->(m:Movie)<-[:DIRECTS]-(d:Director)

       RETURN [a.name](http://a.name) as ActorName, a.DOB as DOB,m.title as MovieTitle,[d.name](http://d.name) as DirectorName |
...
RETURN [p.name](http://p.name) as Person, [c.name](http://c.name) as Project
