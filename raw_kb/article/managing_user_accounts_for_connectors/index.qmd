---
title : Managing User Accounts for Connectors
categories: ['https://domo-support.domo.com/s/topic/0TO5w000000ZamwGAC']
date: 2022-10-24 22:39:00
url : https://domo-support.domo.com/s/article/360042926054
linked_kbs :  ['https://domo-support.domo.com/s/article/360042926274', 'https://domo-support.domo.com/s/knowledge-base', 'https://domo-support.domo.com/s/article/360042926074', 'https://domo-support.domo.com/s/article/360042925494', 'https://domo-support.domo.com/s/article/360043429913', 'https://domo-support.domo.com/s/article/360043429953', 'https://domo-support.domo.com/s/article/360042925814', 'https://domo-support.domo.com/s/article/360043438973', 'https://domo-support.domo.com/s/article/360042926054', 'https://domo-support.domo.com/s/article/360047553253', 'https://domo-support.domo.com/s/article/360043429933', 'https://domo-support.domo.com/s/article/360043429693', 'https://domo-support.domo.com/s/topic/0TO5w000000ZamwGAC', 'https://domo-support.domo.com/s/article/360042934334']
article_id : 000004907
views : 7,033
...
When the OAuth token for an account is about to expire, you receive notifications in Domo and via email/SMS. For information about setting notification preferences, seeÂ [Customizing Notification and Alert Settings](..\configuring_notification_and_alert_settings\index.html "Customizing Notification and Alert Settings").


If you delete a user from Domo and reassign his/her DataSets to another user, the permissions for the DataSet accounts *is not* passed to the new user. The account ownership must be transferred manually.Â For more information, seeÂ [Removing Users from Domo](..\removing_users_from_domo\index.html "Removing Users from Domo").


For more information about the layout of this tab, see [Data Center Layout](..\data_center_layout\index.html "Data Center Layout").
...
The Data Center opens, with theÂ **DataSets**Â tab opened by default.Â 


![DataCetner-DataSet.png](DataCetner-DataSet.png)


2. Click the ![data_center_accounts_icon.png](data_center_accounts_icon.png) icon from the left pane of the screen to open the **AccountsÂ**tab.
...
* By adding a DataSet to Domo and entering new account information (as opposed to selecting an existing account). An entry for the new account appears automatically in the **Accounts** view. This method is discussed in more detail inÂ [Adding a Dataset Using a Data Connector](..\adding_a_dataset_using_a_connector\index.html "Connecting to Data with Connectors").
...
2. ClickÂ ![](2.png) and select **Delete Account**.
...
2. ClickÂ ![](2.png) and select **Re-connect**.
...
When you create accounts, eitherÂ by adding them in theÂ **Accounts** tab or entering credentials for a new DataSet, those accounts take the name of the connector. For example, if you add an account for Facebook, that account is called "Facebook Account."Â Subsequently added accounts for the same connector share this name. This can make it confusing to differentiate between accounts for the same connector. You can eliminate this confusion by givingÂ unique names to accounts in theÂ **Accounts** tab.Â 


**To rename accountsÂ in the Accounts tab,**


1. Click the accountÂ name. (You can also select **Edit account** in the gear menu.)  
The account name and credentials open for editing.
2. EnterÂ a new name for the account.
3. ClickÂ **Save Changes**.Â
...
2. Click ![](2.png) and select **Share account**.


![Share_Account-old.png](Share_Account-old.png)


3. A Share Account access dialog for the the selected account appears, listing the users with access to the DataSets from this account.Â 


4. Click the "Add a person..." search bar to search for the user name you want to share the account with.


![Search_user_dropdown.png](Search_user_dropdown.png)


5. Select the desired user and click **Add**.Â 


6. The newly added user will appear in the **Shared with** section.Â Repeat the same process to add multiple users you want to share the account with.
...
2. Click ![](2.png) and select **Share account**.Â 


3. A dialog appears listing the users with access to DataSets from this account in the **Shared with** section.


4. Click the cross (![Cross.png](Cross.png)) in front of the the user name that you want to remove the access for.Â 


![Remove_user-old.png](Remove_user-old.png)


5. You can remove the account access with multiple users in the same attempt just by clicking the cross (![Cross.png](Cross.png)) in front of the the user names that you want to remove the access for.
...
After transferring ownership, both users can view the associated DataSets in the **Accounts** tab, but only the user with ownership can manage the associated DataSets. If you want to configure the DataSet to use credentials in a different connection account when connecting to the third-party system to get data, change the connection account. For more information, see [Changing the connection account of multiple DataSets](#HowdoImanageuseraccountsforconnectors?-connectionaccount "Managing User Accounts for Connectors").


**Note:Â**Transferring ownership of multiple DataSets requires the following:



* You *must* be the owner of all the DataSets or have an "Admin" default security role or a custom role with the "Manage DataSets" privilege enabled.Â   
Users with a default security role of "Privileged" or "Editor" can only assign ownership for their user-associated DataSets to another user. Users with a default security role of "Admin"Â can assign ownership for any user-associated DataSets. For more information about default security roles, see [Managing Roles](..\managing_custom_roles\index.html "Security Role Reference").
* When transferring ownership in bulk, you *must* assign ownership for *all* DataSets of the associated user.
Â 


**Tip:Â**You can change ownership of a single DataSet in the Details view for the DataSet. For more information, see [Changing the Owner of a DataSet](..\changing_the_owner_of_a_dataset\index.html "Changing the Owner of a DataSet").
**To transfer ownership of DataSets associated with a user,**


1. In **Accounts**, on a listed account, mouse over the **DataSets** column item. A list of the associated DataSets appears.


2. Click **Change Owner**.


![Change_Owner.png](Change_Owner.png)


3. **Transfer DataSets** window pops up. Click **Transfer Ownership**.


4. A **Transfer Ownership** dialog appears.


5. Enter the name of the user to whom you want to transfer ownership of the DataSets and click **Transfer Ownership**.


![Transfer_Ownsership_user_name.png](Transfer_Ownsership_user_name.png)
...
* when permissions are insufficient in the connection accountÂ
...
**Note:Â**Changing the connection account of multiple DataSets, requires the following:



* You *must* be the owner of all the DataSets or have an "Admin" default security role or a custom role in which the "Manage DataSets" privilege is enabled.Â   
...
If you need to consolidate account ownership *before* changing the connection account used for multiple DataSets, first transfer ownership before changing accounts. For more information, see [Transferring ownership of multiple DataSets](#h_01F9BZ162FNRZA734YK8SP3SSV "Managing User Accounts for Connectors").
...
**Tip:Â**You can change the connection account associated with a single DataSet in the Details view for the DataSet.
