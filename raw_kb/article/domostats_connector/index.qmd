...
linked_kbs :  ['https://domo-support.domo.com/s/article/360042926274', 'https://domo-support.domo.com/s/knowledge-base', 'https://domo-support.domo.com/s/article/4577793742615', 'https://domo-support.domo.com/s/article/360042925494', 'https://domo-support.domo.com/s/article/360043439413', 'https://domo-support.domo.com/s/article/360043429953', 'https://domo-support.domo.com/s/article/360043433813', 'https://domo-support.domo.com/s/article/360043429933', 'https://domo-support.domo.com/s/article/360043429693', 'https://domo-support.domo.com/s/topic/0TO5w000000ZamwGAC', 'https://domo-support.domo.com/s/article/360043429913']
...
The DomoStats Connector gives you the freedom to explore how your team is using Domo. Dive in to your team's activities, Projects and Tasks, DataSets, Buzz conversations, and more to find insights and shape your companyâ€™s Domo experience.Â 


You must have an Admin security role to access the DomoStats Connector or to connect to the DomoStats apps. However, once the DataSets have been created or the app has been deployed, they can be shared with anyone in your organization.Â Â Â 
...
To pull in DomoStats DataSets using this Connector, you must have Admin security access. No credentials are required for connecting.Â 
...
This pane contains a singleÂ **Report**Â menu from which you select your desired DomoStats DataSet.
...
| Beast Modes used in CardsÂ  | Returns information about which Beast Modes are currently in use in cards. |
...
| DataFlow Output DataSources | Returns the IDs of outputs from DataFlows.Â  |
| DataFlows | Returns information about DataFlows within your DomoÂ instance, such as DataFlow name, type, status, last updated date, etc.
Â 
...
| DataSets History | Returns DataSetÂ run history information for your Domo instance, such as DataSet name, run start and end time, whether runs were successful, etc.
See [DataSets History DataSet](#h_01GFXVVABHEGEQRSM9A3VA7Y26) below. |
| ML Inference Jobs | Included with data science package. |
| Mobile Activity | Returns a user ID, the user's mobile device, and their session start date and time.Â Â  |
...
| VariablesÂ  | Returns information about existing Variables in your instance. |
...
After a DataSet is created, you can locate the DataSet through filters and tagging. This helps keep DataSets organized and accessible.Â 


Â 


1. In the navigation header, select **Data**.Â 


2. Select **DataSets** from the ![Screen_Shot_2022-07-19_at_2.16.07_PM.png](Screen_Shot_2022-07-19_at_2.16.07_PM.png) navigation rail. Â 


3. In the **Search** bar, type the name of your DataSet. You can also locate DataSets using filters. 


3. In the **Search** bar, select **Add Filter**.Â 


4. Choose **Type** and enter **DomoStats**.


A list of DomoStats DataSets will generate. Choose your DataSet from the list.Â 
...
Locating and organizing your DataSets is made possible by tagging. Tagging groups the DataSets in collections that can be found using the **Tag** filter.Â 


Â 


1. Locate the DataSet in Domo. (See the previous section, Locating DomoStats DataSets, for a step-by-step guide.)Â 


2. Select **+ Add Tag** under the DataSet's name.Â 


![2022-07-19_14-30-16.png](2022-07-19_14-30-16.png)


3. Choose from the list of available tags, or create a new tag using the search feature.Â 


![2022-07-19_14-34-09.png](2022-07-19_14-34-09.png)


4. Select **+** next to **Create new tag.**Â 


5. Select **Save.**Â ![2022-07-19_14-37-14.png](2022-07-19_14-37-14.png)


The DataSet is now tagged and can beÂ found using the Tags filter.Â 
...
| Source\_ID | The ID of the type. For a simplified example, let's say a user added a card to a page. You would get these two actions on an event, and both would be returned on the DataSet.
...
|  |  |
| --- | --- |
| LOGGEDIN | Logged whenÂ someone successfully logs in from the login page using their password. Mobile logins are recorded when a person logs out of the app and back into the app. If a person opens and closes the app, that is not counted toward logins. |
...
| Enabled | The DataFlowÂ has been built but not yet run. |
| Failed | There was an internal problem executing the DataFlow. |
| Running | The DataFlow is currently being executed. |
| Success | Executed successfully. |

 |
| Owner ID | Returns the user ID of the DataFlow owner.Â This ID is the same as the User ID in the DomoStats People DataSet. |
...
| Status | Returns the status for the DataFlowÂ run, either CANCELLED, FAILED, REJECTED, or SUCCESS. |
...
| Owner\_User\_ID | The unique identifying value for the person who owns the DataSet. Â A DataSet owner is either the person who created the DataSet or the person whom ownership was assigned to. |
| Created\_Date | The dateÂ on which the DataSet was run for the first time. |
...
| Execution ID | The ID for a specific run of the DataSet. Â  |
| DatasourceÂ ID | A Domo-specific datasourceÂ ID |
...
| End Time | The time the DataSetÂ finished running, went into an error state, or became invalid. |


#### OKR Objectives DataSet


The fields in this section use "Objectives" and "Key Results".Â Your organization may refer to them as goals, metrics, or something else depending on your Goals Admin Settings. For more information about using the DomoStats Connector with Goals, see [Accessing Goals Data](..\accessing_goals_data\index.md).Â 
...
| Parent IDs | The IDs of the parent objectives if the objective has multiple parents, if applicable.Â  |
...
| Period IDs | The IDs of periodsÂ *ifÂ*an objective is active for multiple periods.Â  |
| Company ID | The ID of the organization that is assigned to the objective. 
(For Company-Type objectives) |
| Company IDs | The ID of each Company-type objective *if* linked to multiple Company-type objectives.Â  |
...
| Expire Date | The end date of the objective.Â  |
...
| Tags | A list of tags associated with the objective. This list is separated by commas.Â  |
...
| Description | The description of the key result.Â  |
...
Â 
CLOSED: The objective that the key result belongs to has been marked complete.Â  |
...
| Card ID | The ID of the card that the key result is linked to (for automatic/card-based key results).Â  |
...
Â 
 Projects and Tasks-based key results: Progress is determined by how many projects/tasks have been completed completed in the Projects and Tasks feature.Â  |
...
| Unit TypeÂ  | The data type of the key result's value and target. Possible values include: 
Â 
NUMERICAL 
PERCENT |
| Project ID | The ID of the project that is linked to the key result (only applicable to Projects and Tasks-based key results).Â  |
...
| Cure Period | Indicates when changes to the key result will no longer be tracked.Â  |
...
If any of the "personal information" fields (name, email, location, etc.) return blank in this DataSet, you can fill in the missing information in the user's Profile page or their page inÂ **Admin Settings > People**.Â For more information, seeÂ [Specifying Your Personal Profile Information](..\specifying_your_personal_profile_information\index.html "Specifying Your Personal Profile Information").
...
| Location | The company location the user has been assigned to.Â  |
...
| Last Login | A timestamp for the last time this user logged in.Â Login is measured by any regular, SSO, or mobile login. To count as a login for mobile, a person must actually log out of the instance and log back in. Opening and closing the app does not count as a login. |
...
| Profile Picture | Returns a "True" value if a profile picture has been added in the user's Profile page or on his/her page inÂ **Admin Settings > Governance > People**. |
| Org Chart | Returns a "True" value if the user has been added as a report on their company org chart in the Profile page.Â  |
| Two Factor Enabled | Returns a "True" value if theÂ company has enabled two-factor authentication as an option in **Admin Settings > Authentication > Authentication**Â and the user has chosen to have two-factor enabled in their Settings menu (available via clicking the Gear menu under their profile picture). |
...
| Project\_id | ID number for the project.Â  |
...
| OwnerName | The name of the person who created the task.Â In the case that the original owner reassigned ownership, then it is the name of the new owner. |
| Priority | The order in which the taskÂ shows up on the task board. Lower priority numbers are above higher priority numbers. |
