...
linked_kbs :  ['https://domo-support.domo.com/s/article/360042925494', 'https://domo-support.domo.com/s/knowledge-base', 'https://domo-support.domo.com/s/article/360042934494', 'https://domo-support.domo.com/s/article/360043429953', 'https://domo-support.domo.com/s/article/360043437733', 'https://domo-support.domo.com/s/article/360043429933', 'https://domo-support.domo.com/s/article/360043429693', 'https://domo-support.domo.com/s/topic/0TO5w000000ZamwGAC', 'https://domo-support.domo.com/s/article/360043429913']
...
1. Navigate toÂ **Admin Settings**Â >Â **Tool Downloads**Â then click theÂ "**Try the Domo CLI**"Â button
...
2. Open a terminal applicationÂ (e.g. Terminal on macOSÂ or PowerShell on Windows)
3. Run the CLI binary file usingÂ Java:Â `java -jar domoUtil.jar`
...
**Important:**Â You must execute the connect command for every new session. Otherwise, you will receive an authentication error. |
...
| `subscribe-user-to-alert` | Subscribes userÂ to an alert. Use 'get-alerts' to get the alert ID. |
...
| `move-page` | Moves pages in navigationÂ hierarchy. |
| `move-data` | Moves data from one Domo DataSet to another. |
| `rename-card` | Renames a card by ID. |
| `backup-card` | Export aÂ  specified card as a JSON blob to a file or stdout that can be used to restore it later. |
...
**Note:**Â Depending on size, the file will be split up as necessary into multiple parts for uploading. We recommend keeping file sizes under 20 MB after compression. |
...
| -pp, --proxypasswordÂ <proxypassword> | Proxy password |
| -ps, --proxyserverÂ <proxyserver> | Proxy server |
| -pt, --proxyportÂ <proxyport> | Proxy port |
| -pu, --proxyuserÂ <proxyuser> | Proxy user |
| -t, --token <TOKEN> | Token |
...
To disconnect from a Domo instance, run the`disconnect`command without using any parameters.Â 
...
1. Create a Domo access token. For more information on how to create and manage access tokens, see [Managing Access Tokens](..\managing_access_tokens\index.html "Managing Access Tokens").Â
2. Obtain the schema file in JSON format of the DataSet you want to upload to Domo by running the`derive-schema`command.


**Note:Â**To derive the schema from an existing CSV file, use the following command, replacing ''filename.csv' with your file name.Â `derive-schema -d 'C:\directory\filename.csv' -s 'C:\directory\schemafile.json'`
3. PerformÂ a pre-check on the data by generating a clean, true CSV with a small number of rows. Obtain the schema file and run the`create-dataset`command to validate the upload will work. You may then delete that DataSet from Domo at your discretion.


**Tip:**Â You will need to place double quotes (") around any commas in your CSV that you want to escape, otherwise the CLI Tool will use the comma as a delimiter.
...
**Important:**Â By default, when updating a DataSet, the entire existing DataSet will be replaced with the new upload unless you use the append parameter.Â
...
Ensure the JDBC driver JAR fileÂ is in the current working directory.
...
**Important:**Â By default, queries return a maximum of one million rows. You can specify a higher limit in your query but you may experience timeouts if the result is too large. For full exports, use the `export-data` command.Â
...
**Note:**Â The exported file respects any Personalized Data Permissions (PDP) that are set for the specified DataSet.
...
The`move-page`commandÂ allows you to move a page up in the hierarchy as a top level page or down in the hierarchy to a subpage.
...
**Tip:**Â For more information on how to find a page ID, see Locating a Page ID.
#### Move Data
...
**Note:**Â To move data using a replace method, remove the -a parameter from the command line.
#### Listing DataSets


The`list-dataset`command will list all of the DataSets based on owner, a search term, or a specific DataSet ID.Â 
...
**Note****:**Â Use 'me' instead of a DataSet Owner ID to use the currently authenticated user. |
...
The`search-domo`commandÂ searches and exports a list from Domo by entity type and search term.Â 
...
The whoami command will return the username and user IDÂ of the individual that is currently logged in. There are no parameters for this command.
...
**Tip:**Â Be sure to update 'uploadfile.script' in the above example to your own script file name.
Â