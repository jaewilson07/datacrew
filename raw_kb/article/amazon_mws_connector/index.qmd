...
linked_kbs :  ['https://domo-support.domo.com/s/article/360042926274', 'https://domo-support.domo.com/s/knowledge-base', 'https://domo-support.domo.com/s/article/360042925494', 'https://domo-support.domo.com/s/article/360043429953', 'https://domo-support.domo.com/s/article/360042926054', 'https://domo-support.domo.com/s/article/360042928314', 'https://domo-support.domo.com/s/article/360043429933', 'https://domo-support.domo.com/s/article/360043429693', 'https://domo-support.domo.com/s/topic/0TO5w000000ZamwGAC', 'https://domo-support.domo.com/s/article/360043429913']
...
To find your Amazon AuthÂ Token and Seller ID, do the following:


1. Navigate to <https://sellercentral.amazon.com/apps/manage> and log in.
2. ClickÂ **Authorize New Developer**.  
   
 ![amazon_mws_1.png](amazon_mws_1.png)
3. Register Domo Inc. with developer ID "181964952887" for JP or developer ID "473744043762" for CA, US, and MX then click **Next**.Â   
   
 ![amazon_mws_2.png](amazon_mws_2.png)
4. Check the box and clickÂ **Next**.  
   
 ![amazon_mws_3.png](amazon_mws_3.png)


You are taken to a screen with your Seller ID, the MWS Auth Token, and the Marketplaces you have access to. You will need all of these to authenticate in theÂ **Credentials**Â pane.Â 
...
This section enumerates the options in theÂ  **Credentials** Â andÂ  **Details** Â panes in the Amazon MWS Connector page.Â The components of the other panes in this page, **Scheduling**Â and **Name & Describe Your DataSet**, are universal across most connector types and are discussed in greater length inÂ [Adding a DataSet Using a Data Connector](..\adding_a_dataset_using_a_connector\index.html "Adding a DataSet Using a Data Connector").


### Credentials Pane


This pane contains fields for entering credentials to connect to your Amazon MWS account. The following table describes what is needed for each field: Â 
...
| Menu | Description |
| --- | --- |
| Report | Select the Amazon MWS report you want to run.Â The following reports are available:
...
| Reports - Schedule List | Returns a list of order report requests that are scheduled to be submitted to Amazon MWS for processing. |
 ...
 |
| Data Selection Mode | Select the method to use to retrieve reports when "Reports - Get Report" is selected in the **Report**Â dropdown menu.Â 
* With theÂ **Easy**Â method, you select your report type from a dropdown menu. The latest version of the selected type will be retrieved automatically.Â
* With theÂ **Advanced**Â method, you enter a report ID into a text field.Â
...
| Report Type | Select the desired report type. Reports must already be generated and available in Amazon MWS.Â  |
| Report Detail Filter (for "Easy" data selection method) | Select the desired report detail filter. |
| Report ID | Enter the generated Report ID for the report you want to import from Amazon MWS. You may also enter the ReportType enumeration value from the Amazon MWSÂ [API documentation](https://developer.amazonservices.com/ "https://developer.amazonservices.com/").Â  |
...
| CSV Report Quote Character | Select the quote character for your CSV report in Amazon MWS. Double quotes are the default. If your report data contains double quotes, try selecting theÂ **Null Character**Â option. |
...
| SKU Selection Method | Select the method you want to use to enter SKUs. If you selectÂ **All**, all available SKUs are returned from your Inventory Supply. If you selectÂ **Discovery**, SKUs are shown dynamically based on the query date selected. If you selectÂ **Advanced**, you will supply the list of SKUsÂ yourself.Â Â Â  |
| Date Report Type | Select the method to use to receive data for date-based reports.Â 
**Current** retrieves data for the last 14 days and appends new data after each run.Â 
...
| Duration | Select whether you want to pull data for a specific date or a date range.Â  |
...
| Days Back | Enter the number of past days that should appear in the report. Specify either today (or 0), yesterday (or 1), or today-7 (or 7) to get data for 7 days into the past. Â  |
...
| Days Back to Start From | Enter the number of the farthest day back that should be represented in the report. Specify either today (or 0), yesterday (or 1), or today-7 (or 7) to get data for 7 days into the past. Combine withÂ **Days Back to End At**Â to create a range of represented days.
...
| Days Back to End At | Enter the number of the most recent day back that should be represented in the report. Specify either today (or 0), yesterday (or 1), or today-7 (or 7) to get data for 7 days into the past. Combine withÂ **Days Back to Start From**Â to create a range of represented days.
For example, if you enteredÂ 10Â forÂ **Days Back to Start From**Â andÂ 5Â forÂ **Days Back to End At**, the report would contain data forÂ 10 days ago up untilÂ 5 days ago. |
| Select Query Start Date | Select the start date for your query. |
| Query Date Type | Select the date type for the query.Â **Created** returns data based on the creation date.Â **Last Updated**Â will return data based on the date of the last update. |
| Select Seller SKUs | Select the seller SKUs you want to retrieve data for. You can select up to 100 SKUs.
Â
**Tip:**Â If you want to bring in more than 100 SKUs, then you will need to select advanced mode and paste the SKU list, comma separated. SKUs can be obtained from the Inventory Report (`_GET_FLAT_FILE_OPEN_LISTINGS_DATA_`).Â   To choose that report in the Amazon MWS connector, select **Reports** > **Get Reports**. Then, select Inventory ReportÂ if using easy mode or paste in the name of the report (`_GET_FLAT_FILE_OPEN_LISTINGS_DATA_`) if using advanced mode.Â  For more information on Amazon report types, seeÂ [https://docs.developer.amazonservices.com/en\_US/reports/Reports\_ReportType.html](https://docs.developer.amazonservices.com/en_US/reports/Reports_ReportType.html "https://docs.developer.amazonservices.com/en_US/reports/Reports_ReportType.html") |
| Seller SKU List | Enter the list of SKUs you want to retrieve data for, separated by commas.Â  |
| Continue on Invalid SKU Error? | SelectÂ **Skip Invalid SKUsÂ**if you want to continue your DataSet import even if invalid SKUs are discovered. SelectÂ **Warn about Invalid SKUs**Â if you want the import to stop and provide an error message if invalid SKUs are discovered.Â  |
...
| Report ID | Enter the Report Type or the generated Report ID you want to get data for. Specific Report IDs can be retrieved using the **List Reports** report.Â  |
...
##### Can Domo schedule Amazon MWSÂ Reports on my behalf?
