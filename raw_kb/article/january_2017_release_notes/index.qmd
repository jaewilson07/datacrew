...
url : https://domo-support.domo.com/s/article/360043439593
linked_kbs :  ['https://domo-support.domo.com/s/article/360042925494', 'https://domo-support.domo.com/s/knowledge-base', 'https://domo-support.domo.com/s/article/360042923274', 'https://domo-support.domo.com/s/article/360043429953', 'https://domo-support.domo.com/s/article/360043439593', 'https://domo-support.domo.com/s/article/360042934614', 'https://domo-support.domo.com/s/article/360043429933', 'https://domo-support.domo.com/s/article/360043429693', 'https://domo-support.domo.com/s/topic/0TO5w000000ZamwGAC', 'https://domo-support.domo.com/s/article/360043428733', 'https://domo-support.domo.com/s/article/360043429913']
article_id : 000004447
views : 2,060
...
You can now change the chart type for most charts right in the Details page instead of having to first open the Card Builder. You do this by clicking ![details_chart_picker_icon.png](details_chart_picker_icon.png) then selecting the new chart. In contrast to the chart type picker in Card Builder, which only shows icons for each chart type, the Details view chart type picker shows you actual thumbnail versions of each available chart type *with your chart data applied*. So, for example, if you opened the Details page for the following Vertical Stacked Bar chart...  
Â 
...
The chart type picker only displays thumbnails for chart types that are compatible with your data. In the previous example, "Scatter Plot" is not suggested as a chart type because Scatter Plots have a different data structure than Vertical Stacked Bars (i.e. two value columns instead of one). If your chart has a chart type not compatible with any other charts (such as a Calendar chart), theÂ ![details_chart_picker_icon.png](details_chart_picker_icon.png) icon is grayed out.


The new chart type picker is *not* available for Maps.
...
You can also toggle legend items in Pie-type charts. Legend toggling works a bit differently for these charts. In this case, when you click a legend item, it disappears from the legend (there is no empty square to indicate that the item is hidden). However, you can then click the ![](6.png) icon next to the chart to reset it to normal.


Â
**Note:** Be aware that if you hide data in a chart by toggling a legend item, it is still hidden when other users go into the Details view; however, they can show it again by clicking that legend item. If you want to hide data from users, you should use [PDP](..\creating_and_deleting_pdp_policies\index.html "Filtering Data in a Card Details View") instead.
Chart Zoom
----------
...
![chart_zoom.png](chart_zoom.png)


You could then click the ![](6.png) icon next to the chart to zoom back out to the normal version.
...
For more information about sorting, see [Sorting the Data in Your Chart](..\sorting_the_data_in_your_chart\index.html "Sorting the Data in Your Chart").
...
When you make changes to a chart in the Details view (whether by using any of the above features or filtering using the Analyzer tool), you can now save a copy of the edited chart to your Domo. You do this by clicking the ![](15.png) icon in the Details toolbar then selecting **Save As**. You can then give the new version of the card a new name, enter a description to help viewers understand what has been changed in the card, and select the page in Domo where the new card will reside. Duplicated cards retain all Analyzer, Chart Properties, and date grain selections.
...
You can also create a "Save As" version of a card in the card view. For more information, see [Duplicating Cards](..\duplicating_cards_save_as\index.html "Duplicating Cards").


**To create a "Save As" version of a card in the Details view,**


1. Select Â ![](15.png) **> Save As**.
2. (Optional) Change the title of the card if desired. (All duplicated cards incorporate the "Copy of" text in the name by default). Â
...
1. Open the Card Builder for the line or bar chart you want a forecast for.
2. In **Chart Properties**, click **Last Value Projection**.
3. In the **Project Using** menu, select **Forecast Value**.
