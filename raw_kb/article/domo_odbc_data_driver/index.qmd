...
linked_kbs :  ['https://domo-support.domo.com/s/article/360042925494', 'https://domo-support.domo.com/s/knowledge-base', 'https://domo-support.domo.com/s/article/360043437693', 'https://domo-support.domo.com/s/article/360043429953', 'https://domo-support.domo.com/s/article/360043429933', 'https://domo-support.domo.com/s/article/360043429693', 'https://domo-support.domo.com/s/topic/0TO5w000000ZamwGAC', 'https://domo-support.domo.com/s/article/360043429913']
...
Domo ODBC Data Driver requires a Client ID and Client Secret for authentication, and it supports simple queries as described later in this document.Â 


To obtain Domo ODBC Data Driver, go to **Admin** >Â **Tool Downloads**.
...
ODBC drivers follow thisÂ format for the connection string:
...
Domo ODBC Data Driver supports standard SQL-type statements in the formatÂ SELECT <column names, or \*> FROM <dataset id> WHERE <clause> â€¦ etc.Â For example:  
Â   
...
Â
2. (Conditional) If you donâ€™t have a client, create a new one for the ODBC driver by doing the following:


	1. ClickÂ **Create a Client**.  
	  
	![DDD-2.png](DDD-2.png)
	2. Enter the name and description.
	3. Select application scopes. (**Data**Â is a mandatory scope.)
...
Before you start, you'll need:Â 
...
If your system doesnâ€™t have all the prerequisites, the installer will download and install them. You need to grant permission for the installation of the .NET framework and VC++ runtime. If you have all the prerequisites, you will see the Domo ODBC Data Driver Installation wizard.
...
Once the installation is done, you should check to see if the driver is working.Â 
...
For 32-bit Domo ODBC Data Driver, you mustÂ openÂ the 32-bit version of the ODBC Data Sources, and 64-bit Domo ODBC Data Driver for the 64-bit version. These versions are only available on 64-bit machines. On 32-bit machines, there will be only one version of the Administrator.  
  
The path of ODBC Data Source Administrator isÂ %systemdrive%\Windows\SysWoW64\odbcad32.exe for 32-bit and %systemdrive%\Windows\System32\odbcad32.exe for 64-bit.Â   
  
This application is used for configuring DSNs and requires administrative permissions on the system.
2. Go to the **File DSN**Â tab and click **Add**.Â
...
In this case, you mustÂ verify the installation, credentials, network, and accessibility to Domo APIs.Â Otherwise, a file will get created on successful verification.  
  
![DDD-19.png](DDD-19.png)  
  
Proceed only if your files get created successfully.Â
...
You shouldÂ see a file called "DomoDataDSII."  
  
![DDD-20.png](DDD-20.png)
2. Click **Configure**.  
A dialog asksÂ forÂ your Client ID and Client Secret.
...
Instead of configuring the DomoDataDSII, you can also create your own DSN with the **Add**Â button. You can create as many DSNs as you need.
...
1. In the **Data**Â tab in Excel, selectÂ **New Query > From Other Sources >Â From ODBC**.  
  
![DDD-22.png](DDD-22.png)
2. Select "DomoDataDSII" (orÂ the name of your DSN) from the dropdown.
3. In **Advanced Options**,Â enter theÂ "Select"Â query (see [Query Syntax](#h_d1ce073e-dcdf-46f3-a93c-d842b3c41b20 "DomoDataDriver")), then click **OK**. Â   
  
![DDD-23.png](DDD-23.png)  
  
A preview will load. Note that this mayÂ takeÂ a while for a large DataSet.)  
  
![DDD-24.png](DDD-24.png)
4. Click **Load** and data will be loaded after execution of the query.Â Â   
...
Follow the same steps mentioned in [Using DSN](#h_cbf25764-428d-4423-a820-d9628f1811ef "DomoDataDriver") to open ODBC settings. However, instead of selecting "DomoDataDSII" in step 2,Â select **None**Â in **Data Source Name**. Then enter the connection string as described in [Connection Strings](#h_737894a0-3bb4-4605-aed1-645c5a0a7e31 "DomoDataDriver")Â and the query as described in **Query**.
...
* The Bitness of the tool and the driver should match. The 64-bit tool will work with the 64-bit driver, and the 32-bit tool will work with the 32-bit Domo ODBC Data Driver. Please make sureÂ you are using the correct products.Â
...
1. Select **ODBC** and click **Next**.Â
2. Type "DomoDataDSII"Â in the **Database/Schema** box.
3. Click **Test Connection.**  
You shouldÂ see the test results.
...
Once query execution is complete, youÂ shouldÂ see the results.  
