...
linked_kbs :  ['https://domo-support.domo.com/s/article/360043429693', 'https://domo-support.domo.com/s/article/360043429933', 'https://domo-support.domo.com/s/article/360043429913', 'https://domo-support.domo.com/s/knowledge-base', 'https://domo-support.domo.com/s/article/360045485833', 'https://domo-support.domo.com/s/article/360042925494', 'https://domo-support.domo.com/s/article/360055259234', 'https://domo-support.domo.com/s/article/4662511363351', 'https://domo-support.domo.com/s/article/360043429953', 'https://domo-support.domo.com/s/article/360047553253', 'https://domo-support.domo.com/s/topic/0TO5w000000ZamwGAC', 'https://domo-support.domo.com/s/article/360047787514', 'https://domo-support.domo.com/s/article/360042922874']
...
* If you do not have contact information for your CSM, TC, or AE, contact Technical Support for information on how to contact Support, please see: [Getting Help.](..\getting_help\index.qmd)
...
In the **Schema** tab, you tell the tile what the results of your script looks like. To use this, click **Schema** in the tile editor panel toolbar. For a deep dive into this tab, see the [Populating the Resulting Schema](#h_939758a7-1e71-44f2-956c-0db0ad9b414e "Scripting Actions in ETL") section of this documentation.
...
1. Click either the ![scripting_actions_preview_icon.png](scripting_actions_preview_icon.png) button on the right side of the tile editor panel toolbar or the **Run Preview** button in the toolbar at the top of the canvas. (If there is an issue with your preview, see the [Troubleshooting](#h_fbf96559-0d59-47a4-a8d9-096c5e39abeb "Scripting Actions in ETL") section of this documentation.)  
...
If you have not done so already, check out the [Take a Tour](#h_8d005f8d-2ee4-4e2e-b9d4-8b09947fed7a "Scripting Actions in ETL") section of this article to become familiar with the different features of the scripting tile.
...
4. (Conditional) If the editor panel is not open at the bottom of the screen, click the Scripting tile you just added to the canvas so that it is selected; the editor panel will then open.  
 ...
 The code editor will be auto-populated with an initial code template. For more information on what is included in the template, see the [Getting Data In and Out of the Scripting Tile](#h_b5ed4d51-1f33-4fb5-9e9e-1c16369a9e59 "Scripting Actions in ETL") section of this documentation. A yellow banner will also be displayed. Ignore this for now; it will be addressed later on in the configuration steps.  
...
7. To provide the tile with this information...
	...
	2. Choose one of the methods listed in the [Populating The Resulting Schema](#h_939758a7-1e71-44f2-956c-0db0ad9b414e "Scripting Actions in ETL") section of this article and follow the steps there. Afterwards, continue the rest of the steps listed below.
...
| --- | --- | --- |
| read\_dataframe(input\_name=None) | *input\_name:*Name of the Magic ETL input tile. Not required if only one input is available. | A Pandas DataFrame representing the tiles input.  |
| write\_dataframe(dataframe) | *dataframe:* Pandas DataFrame that will be used as the output for the Magic ETL tile.  | N/A |
| read\_array(column\_name,input\_name=None) | *column\_name*: Name of the column.  *input\_name:*Name of the Magic ETL input tile. Not required if only one input is available. | A single column from the input as a NumPy array. |
| read\_array\_dict(input\_name=None) | *input\_name:*Name of the Magic ETL input tile. Not required if only one input is available. | The input as an OrderedDict of column names mapping to NumPy arrays. |
...
| Method Signature | Parameters | Returns |
| --- | --- | --- |
| read.dataframe([input.name](http://input.name)=NULL, see below) | [input.name](http://input.name)*:*Name of the Magic ETL input tile. Optional if only one input is available. | The tiles input data.frame. |
...
* If your tile is not marked as configured after writing the script, make sure you have provided a resulting schema. See the [Populating The Resulting Schema](#h_939758a7-1e71-44f2-956c-0db0ad9b414e "Scripting Actions in ETL") section of this article for more info.
...
* If a script is failing, check that you have properly finished all three of the steps listed in the [Getting Data In and Out of the Scripting Tile](#h_b5ed4d51-1f33-4fb5-9e9e-1c16369a9e59 "Scripting Actions in ETL") section of this article. Double-check the variable name that you are writing back to Domo to make sure it is correct.
